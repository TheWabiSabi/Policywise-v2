...
// Common country codes for the phone input (frontend display only).
const countryCodes = [
...
function getGoogleErrorMessage(err) {
  ...
  if (
    ...
    lower.includes("cognito authentication failed")
  ) {
...
export default function Auth({ onAuthSuccess }) {
  ...
  const handleKeyDown = (e) => {
    ...
    const inputs = Array.from(form.elements).filter(
      (el) =>
        (el.tagName === "INPUT" || el.tagName === "SELECT") && !el.disabled,
    );
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      toast.error(
        getGoogleErrorMessage(err) || "Google sign in is not available.",
      );
    }
  };
  ...
  const handleSignUp = async (e) => {
    ...
    setLoading(true);
    try {
      // Note: phone number is intentionally not sent to the backend — it's
      // collected here for frontend/UX purposes only.
      await auth.signUp({
        email: email.trim(),
        password,
  ...
  const inputCls = (hasError) =>
    `appearance-none block w-full pl-10 pr-3 py-3 border rounded-xl focus:outline-none focus:ring-2 transition-all sm:text-sm ${
      hasError
  ...
  const phoneInvalid = phoneNumber.length > 0 && phoneNumber.length !== 10;
      ...
      <div className="hidden lg:flex lg:w-1/2 relative bg-gradient-to-br from-blue-900 via-indigo-900 to-slate-900 overflow-hidden items-center justify-center p-12">
        ...
        <div className="relative z-10 max-w-lg text-white">
          ...
          <p className="text-lg text-blue-100/80 mb-10 leading-relaxed">
            Instantly analyze, compare, and extract critical coverage details
            from dense health insurance policy documents using cutting-edge AI.
          </p>
          ...
          <div className="space-y-5">
            {[
              {
                Icon: Zap,
                title: "Instant Precision Parsing",
                desc: "Extract exact coverage limits, sum insured, and Super Credit bonuses instantly.",
              },
              {
                Icon: FileSearch,
                title: "Smart Policy Comparison",
                desc: "Effortlessly compare features and exclusions across multiple health documents.",
              },
              {
                Icon: ShieldCheck,
                title: "Regulatory Context",
                desc: "Automatically identifies standard IRDAI-approved features and rights.",
              },
            ].map(({ Icon, title, desc }) => (
              ...
              <div className="mb-8">
                ...
                <div className="mt-6 flex p-1 bg-slate-100 rounded-lg">
                  <button
                    ...
                    type="button"
                    className="flex-1 py-2 text-sm font-semibold rounded-md bg-white shadow text-blue-700 transition-all duration-200"
                  >
                    Login
                  </button>
                  <button
                    ...
                    type="button"
                    className="flex-1 py-2 text-sm font-semibold rounded-md text-slate-500 hover:text-slate-700 transition-all duration-200"
                  >
                    Sign Up
                  </button>
                </div>
          ...
          {view === "signup" && (
            <>
              <div className="mb-8">
                ...
                <p className="text-slate-500 text-sm">
                  Join PolicyWise to start analyzing policies.
                </p>
                <div className="mt-6 flex p-1 bg-slate-100 rounded-lg">
                  <button
                    onClick={() => switchView("signin")}
              ...
              >
                ...
                {/* Phone number (frontend only — not sent to backend) */}
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">
                    Phone Number{" "}
                  ...
                  <div className="flex gap-2">
                    <select
                      ...
                      className="appearance-none flex-shrink-0 w-[104px] px-2 py-3 border border-slate-200 rounded-xl bg-slate-50 focus:bg-white text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all sm:text-sm"
                    >
